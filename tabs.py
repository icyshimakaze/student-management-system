...
from services import PermissionDenied, ServiceBundle, ValidationError
...
def configure_table(table: QTableWidget, stretch_columns: tuple[int, ...] = ()) -> None:
    ...
    # Columns without explicit stretch still shrink/grow with a sensible floor,
    # so headers are never truncated to a few letters.
    for col in range(table.columnCount()):
        if col not in stretch_columns:
...
class DashboardTab(QWidget):
    def __init__(self, services: ServiceBundle, is_admin: bool):
        ...
        self.course_table=QTableWidget(); self.course_table.setColumnCount(4); self.course_table.setHorizontalHeaderLabels(["Course","Name","Enrolled","Average grade"]); configure_table(self.course_table,(0,1))
        self.recent_table=QTableWidget(); self.recent_table.setColumnCount(4); self.recent_table.setHorizontalHeaderLabels(["Date","Student","Course code","Course"]); configure_table(self.recent_table,(1,3))
        self.grade_table=QTableWidget(); self.grade_table.setColumnCount(2); self.grade_table.setHorizontalHeaderLabels(["Grade band","Count"]); configure_table(self.grade_table,(0,))
        ...
        for table in (self.course_table,self.recent_table,self.grade_table): table.setMinimumHeight(220)
        cards_and_grid=QWidget(); inner=QVBoxLayout(cards_and_grid); inner.addWidget(title); inner.addLayout(self.cards); inner.addLayout(grid)
        # Scrollable so every section stays reachable even on small screens.
        scroll=QScrollArea(); scroll.setWidgetResizable(True); scroll.setFrameShape(scroll.Shape.NoFrame); scroll.setWidget(cards_and_grid)
