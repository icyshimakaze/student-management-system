...
class StudentRepository(BaseRepository):
    ...
    def list(self, search: str = "") -> list[dict[str, Any]]:
        ...
        """
        return self.fetch_all(sql, (term, pattern, pattern, pattern, pattern))
...
class CourseRepository(BaseRepository):
    def list(self, teacher_id: int | None = None, search: str = "") -> list[dict[str, Any]]:
        term = search.strip()
        p = f"%{term}%"
        teacher_clause = "" if teacher_id is None else " AND c.teacher_id=%s"
        params: tuple[Any, ...] = (term, p, p, p) + (() if teacher_id is None else (teacher_id,))
        return self.fetch_all(
            f"""SELECT c.course_id,c.course_code,c.course_name,c.credits,c.teacher_id,
