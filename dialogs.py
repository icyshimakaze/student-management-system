...
class CourseDialog(ValidatedDialog):
    def __init__(self,parent=None,teachers=None,course=None):
        ...
        if course:
            self.course_code_input.setText(course.get("course_code","")); self.course_name_input.setText(course.get("course_name","")); self.credits_input.setValue(int(course.get("credits",3))); idx=self.teacher_combo.findData(course.get("teacher_id")); self.teacher_combo.setCurrentIndex(max(idx, 0))
